...
-- 
-- As 'Modify' is an open type family, there should only be a single instance of 'Modify' (unfortunately).
-- 
-- That instance will be used for modification of record selectors of a type wrapped into 'Servant.Record.RecordParam'.
-- 
-- Modules that use the 'Modify' type family must be imported together with a module that defines an instance of 'Modify'.
-- 
-- - 'Servant.Record' uses 'Modify' to modify the record selectors.
--
-- Here's an example of a 'Symbol' modification.
-- 
