...
type family UnRecordParam (mkExp :: Symbol -> Exp Symbol) (x :: Type) :: Type where
  ...
  UnRecordParam mkExp (RecordParam a) = UnRecordParam mkExp (Rep a ())
...
--
-- @
-- instance (Generic a, GHasLink mkExp (Rep a) sub) => HasLink (RecordParam a :> sub) where
...
-- @
genericToLink :: forall mkExp a sub b. (Generic a, GHasLink mkExp (Rep a) sub) => (Link -> b) -> Proxy (RecordParam a :> sub) -> Link -> a -> MkLink sub b
genericToLink toA _ l record = gToLink (Proxy :: Proxy mkExp) toA (Proxy :: Proxy sub) l (from record :: Rep a ())

data GParam a

instance GHasLink mkExp a sub => HasLink (GParam (a ()) :> sub) where
...
instance
  ( HasLink sub
  , GHasLink mkExp a (GParam (b ()) :> sub)
  ) =>
  GHasLink mkExp (a :*: b) sub
  where
  gToLink _ toA _ l (a :*: b) =
    gToLink (Proxy :: Proxy mkExp) toA (Proxy :: Proxy (GParam (b ()) :> sub)) l a b
  {-# INLINE gToLink #-}

instance
  (GHasLink mkExp a sub, HasLink sub) =>
