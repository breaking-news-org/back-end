...
import Persist.Types.User (AuthorName)
  ...
  RepoSelectNews filters -> do
    ...
    news <- withConn $ select do
      ...
      let
        ...
        createdUntil = news.creationDate <=. val (filters ^. #_filters_createdUntil . non now)
        createdSince = news.creationDate >=. val (filters ^. #_filters_createdSince . non thisYear)
        mkSql :: Getting (Maybe a) (Filters Maybe) (Maybe a) -> (a -> SqlExpr (Value Bool)) -> SqlExpr (Value Bool)
        mkSql l f = maybe (val True) f (filters ^. l)
        createdAt = mkSql #_filters_createdAt (\x -> news.creationDate ==. val x)
        userId = mkSql #_filters_authorName (\x -> users.authorName ==. val x)
        category = mkSql #_filters_category (\x -> news.category ==. val x)
        newsText = mkSql #_filters_content (\x -> val x `like` news.text)
